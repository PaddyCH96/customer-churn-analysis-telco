...
def load_df():
    ...
    # --- Standardize churn into a boolean (works whether DuckDB inferred bool or string) ---
    s = df["Churn"]
    if s.dtype == bool:
        df["churned"] = s
    else:
    ...
    # Clean up a few categoricals (whitespace / consistent display)
...
with tab1:
    ...
    fig = px.bar(
        by_contract, x="Contract", y="churn_rate",
    ...
    fig = px.bar(
        by_payment, x="PaymentMethod", y="churn_rate",
    ...
    by_addons = (
        f.groupby(["TechSupport", "OnlineSecurity"], as_index=False)
         .agg(customers=("customerID", "count"), churn_rate=("churned", "mean"))
         .sort_values("churn_rate", ascending=False)
    )

    fig = px.imshow(
        by_addons.pivot(index="TechSupport", columns="OnlineSecurity", values="churn_rate"),
        text_auto=".1%",
        aspect="auto",
        title="Churn rate heatmap: TechSupport × OnlineSecurity",
...
with tab2:
    ...
    fig = px.line(
        by_tenure, x="tenure_bucket", y="churn_rate", markers=True,
        hover_data={"customers": True, "churn_rate": ":.3f"},
        labels={"tenure_bucket": "Tenure bucket (months)", "churn_rate": "Churn rate"},
        title="Churn rate by ten
